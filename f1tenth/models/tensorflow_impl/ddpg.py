...
class OrnsteinUhlenbeckActionNoise:
    ...
    def __init__(self, mean, std_dev=0.2, theta=0.15, dt=1e-2, x_initial=None):
        self.theta = theta
        self.mean = mean
...
class Actor(tf.keras.Model):
    ...
    def __init__(self, action_size):
        ...
        self.actor = tf.keras.layers.Dense(action_size, activation='tanh', kernel_initializer=tf.random_uniform_initializer(minval=-0.003, maxval=0.003))
...
class Agent:
    ...
    def __init__(self,
        ...
        self.target_actor = Actor(action_size=action_size)
    ...
    def train(self, env, batch_size=64, max_episode=float('inf')):
        upper_bound = env.action_space.high[0]
        lower_bound = env.action_space.low[0]
        noise = OrnsteinUhlenbeckActionNoise(mean=np.zeros(1), std_dev=float(self.std_dev) * np.ones(1))
        buffer = Buffer(num_states=env.observation_space.shape[-1], num_actions=env.action_space.shape[-1], batch_size=batch_size)
        ...
        for episode in count(1):
            ...
            while True:
                ...
                action = self._get_action(state_tf, lower_bound=lower_bound, upper_bound=upper_bound, noise=noise)
            ...
            print(f'Episode #{episode} Average reward: {average_rewards[-1]:.4f} (Loss: {np.mean(episode_losses):.6f})')

        env.close()

    def _get_action(self, state, lower_bound=-1.0, upper_bound=1.0, noise=lambda: 0):
        sampled_actions = tf.squeeze(self.actor(state)).numpy() + noise()
        legal_actions = np.clip(sampled_actions, lower_bound, upper_bound)
        return np.asarray([legal_actions.squeeze()])
...
def main():
    ...
    agent = Agent(action_size=env.action_space.shape[-1], std_dev=args.std_dev, gamma=args.gamma, tau=args.tau)
    agent.train(env)
